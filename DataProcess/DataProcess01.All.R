...
DataProcess01.Raw <- function() {
  ...
  for(i in 1:dirs.length) {
    ...
    #Launch price (or the oldest record available)
    ...
    dataset.all$MultiPlayer[i] = "Multi-player" %in% categories
    dataset.all$Coop[i] = "Co-op" %in% categories
    dataset.all$LocalCoop[i] = "Local Co-op" %in% categories
    ...
    sc.table <- data.frame(year = as.numeric(as.character(gsub(".+(?= [0-9]{4})", "", sc.table.raw$Month, perl = TRUE))), 
                           ...
                           avg = as.numeric(as.character(sc.table.raw$`Avg. Players`)), peak = as.numeric(as.character(sc.table.raw$`Peak Players`)))
