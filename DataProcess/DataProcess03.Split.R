...
DataProcess03.Split <- function(split = c(0.60, 0.20, 0.20), intervals = c(2,10,30,80,250,750,2000,5000,15000), mode = "reg", evaluation = FALSE
                                , new.game = NULL, thumbnail = NULL, min.devpub = 2, seed = 1, top.terms = 50) {
  source("./DataProcess/DataProcess04.Final.R")
  
  if(evaluation && (length(split) != 3 || !identical(sum(split), 1))) {
  ...
  screenshots.colors <- screenshots.colors[,-c(which(colnames(screenshots.colors) == "SColorsFull"):which(colnames(screenshots.colors) == "SColorHue16"))]
  ...
  if(evaluation) {
    ...
    process.final(dataset.train, type = "train", mode = mode, evaluation = evaluation, min.devpub, seed = seed, top.terms = top.terms)
    cat("\n")
    cat("Processing validation data:", "\n")
    process.final(dataset.val, type = "val", mode = mode, evaluation = evaluation, min.devpub, seed = seed)
    cat("\n")
    cat("Processing test data:", "\n")
    process.final(dataset.test, type = "test", mode = mode, evaluation = evaluation, min.devpub, seed = seed)
    return(NULL)
  } else {
    dataset.new.processed <- process.final(dataset.new, type = "test", mode = mode, evaluation = evaluation, min.devpub, seed = seed)
