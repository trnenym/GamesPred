...
DataProcess02 <- function(evaluation = FALSE, game.predicted = NULL) {
  ...
  sequel.tags <- c("sequel", "franchise", "continuation")
  intersects.sequel <- sapply(dataset$Description, function(x) is.token.intersect(x, sequel.tags, ignore.case = TRUE))

  dataset$IsSequel <- intersects.sequel | is.sequel(dataset$Name)
  ...
  for (i in 1:nrow(dataset)) {
    ...
    dataset$SimplifiedChinese[i] <- "Simplified Chinese" %in% languages
