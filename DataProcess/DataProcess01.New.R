...
DataProcess01.New <- function(data.raw) {
  ...
  game.predicted$CrossPlatformMP <- "cpmp" %in% data.raw$Features
  game.predicted$LocalMP = "lmp" %in% data.raw$Features
  game.predicted$Coop = "co" %in% data.raw$Features
  game.predicted$OnlineCoop = "oco" %in% data.raw$Features
  game.predicted$SplitScreen = "ss" %in% data.raw$Features
  game.predicted$SteamAchievements = "ach" %in% data.raw$Features
  game.predicted$SteamLeaderboards = "lead" %in% data.raw$Features
  game.predicted$SteamTradingCards = "cards" %in% data.raw$Features
  game.predicted$SteamWorkshop = "wshop" %in% data.raw$Features
  game.predicted$LevelEditor = "le" %in% data.raw$Features
  game.predicted$PartialControllerSupport = "pcon" %in% data.raw$Features
  game.predicted$FullControllerSupport = "fcon" %in% data.raw$Features
  game.predicted$VRSupport = "vr" %in% data.raw$Features
  game.predicted$SteamCloud = "cloud" %in% data.raw$Features
  game.predicted$ValveAntiCheat = "vac" %in% data.raw$Features
  game.predicted$CaptionsAvailable = "cap" %in% data.raw$Features
  game.predicted$CommentaryAvailable = "com" %in% data.raw$Features
  game.predicted$InAppPurchases = "iap" %in% data.raw$Features
