...
# Evaluation.Reg <- function() {
  ...
  # model1 <- randomForest(Players ~ ., data = dataset.train1, ntree = 500, mtry = 8)
  ...
  difs1 <- difs1[indices.repeated]


  # dataset.train2$Players <- numeric(length(difs1))
  ...
  # model2 <- randomForest(Players ~ ., data = dataset.train2, ntree = 500, mtry = 8)
  ...
  predictions <- predict(model1, newdata = dataset.test)

  # difs2 <- abs(predictions - actual)
  # difs2.factor <- numeric(length(difs2))
  ...
  # cm <- confusionMatrix(results2$predicted, results2$actual, positive = "1")
  # print(cm$byClass)
  # print(cm$table)
